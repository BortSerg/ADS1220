...
#define EXTERNAL            0x01        // External interrupt from scetch
...
private:
        ...
        uint8_t ads_pga = 1;
        float vref = 2.048;                       // Internal ref voltage ADS1220 (default)
        float vfsr = vref/ads_pga;
...
public:
        ...
        float ConvertToMilivolt(int32_t ads_value);
        // SPI commands
        void Start (void);
        void Reset (void);
        void PowerDown (void);
